...
*/
  ...
  typedef struct ide_drive {
    ...
    uint8_t               drive_nr;            // Master drive (IDE_DRIVE_MASTER) or slave drive (IDE_DRIVE_SLAVE)
  ...
  typedef struct ide_channel {
    uint8_t                  channel_nr;       // Is this a primary or secondary channel (IDE_CHANNEL_PRIMARY / IDE_CHANNEL_SECONDARY)
    struct ide_controller  *controller;      // Pointer to the controller this channel is on

    uint16_t                 base;             // Base port address
    uint16_t                 dev_ctl;          // Device port address
    uint8_t                  no_int;           // Wheter or not this channel has an IRQ
  ...
  uint32_t ide_sector_read (ide_drive_t *drive, uint32_t lba_sector, uint32_t count, char *buffer);
