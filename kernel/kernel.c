...
void kentr(uint32_t magic, struct multiboot_info* boot_info) {
    ...
    pixel(boot_info, 10, 10, 0x00);
    pixel(boot_info, 10, 11, 0x00);
    pixel(boot_info, 10, 12, 0x00);
    pixel(boot_info, 10, 13, 0x00); pixel(boot_info, 11, 13, 0x00);
    pixel(boot_info, 10, 14, 0x00);
    pixel(boot_info, 10, 15, 0x00);
    pixel(boot_info, 10, 16, 0x00);

    pixel(boot_info, 12, 10, 0x00);
    pixel(boot_info, 12, 11, 0x00);
    pixel(boot_info, 12, 12, 0x00);
    pixel(boot_info, 12, 13, 0x00);
    pixel(boot_info, 12, 14, 0x00);
    pixel(boot_info, 12, 15, 0x00);
    pixel(boot_info, 12, 16, 0x00);

    display_text(boot_info, "╘╡Welcome to AsterOS 0.7!╞╛", 0, 0, 0x0f);
    display_text(boot_info, "This is the working VBE terminal.", 0, 8, 0x07);
...
void pixel(struct multiboot_info* mboot, int x, int y, uint16_t color)
{
...
void print_char(struct multiboot_info* boot_info, char c, int x, int y, uint16_t color) {
    if (c < 0 || c > 127) return; // Проверка на допустимый диапазон символов
    ...
    for (int row = 0; row < FONT_HEIGHT; row++) {
        for (int col = 0; col < FONT_WIDTH; col++) {
            ...
            if (char_data[row] & (1 << col)) { // Изменено с (FONT_WIDTH - 1 - col) на col
...
void display_text(struct multiboot_info* boot_info, const char *text, int start_x, int start_y, uint16_t color) {
    ...
    while (*text) {
        print_char(boot_info, *text, x, y, color);
        x += FONT_WIDTH; // Переход к следующему символу
        if (x >= boot_info->framebuffer_width) { // Если достигли конца строки
            x = start_x; // Сбросить x
            y += boot_info->framebuffer_height; // Переход к следующей строке
        }
        text++;
    }
}