...
static uint32_t volume_bitmask;
...
static uint8_t mount_buffer[512];

/// UCS-2 offsets used in long file name (LFN) entries
static const uint8_t lfn_lut[] = {1, 3, 5, 7, 9, 14, 16, 18, 20, 22, 24, 28, 30};
...
/// Private prototypes
static void fat_memcpy(const void* src, void* dest, uint32_t count);
static uint8_t fat_memcmp(const void* src_1, const void* src_2, uint32_t count);
static void fat_store32(void* dest, uint32_t value);
static void fat_store16(void* dest, uint16_t value);
static uint32_t fat_load32(const void* src);
static uint16_t fat_load16(const void* src);
static uint8_t fat_volume_add(struct volume_s* vol);
static uint8_t fat_volume_remove(char letter);
static uint8_t fat_search(const uint8_t* bpb);
static void fat_kprint_sector(const uint8_t* sector);
static uint8_t fat_dir_lfn_cmp(const uint8_t* lfn, const char* name, uint32_t size);
static uint8_t fat_dir_sfn_cmp(const char* sfn, const char* name, uint8_t size);
static uint8_t fat_dir_sfn_crc(const uint8_t* sfn);
static uint8_t fat_dir_set_index(struct dir_s* dir, uint32_t index);
static uint8_t fat_dir_get_next(struct dir_s* dir);
static uint8_t fat_dir_search(struct dir_s* dir, const char* name, uint32_t size);
static uint8_t fat_table_get(struct volume_s* vol, uint32_t cluster, uint32_t* fat);
static uint8_t fat_table_set(struct volume_s* vol, uint32_t cluster, uint32_t fat_entry);
static uint8_t fat_read(struct volume_s* vol, uint32_t lba);
static uint8_t fat_flush(struct volume_s* vol);
static inline uint32_t fat_sect_to_clust(struct volume_s* vol, uint32_t sect);
static inline uint32_t fat_clust_to_sect(struct volume_s* vol, uint32_t clust);
static fstatus fat_follow_path(struct dir_s* dir, const char* path, uint32_t length);
static fstatus fat_get_vol_label(struct volume_s* vol, char* label);
void fat_kprint_info(struct info_s* info);
static uint8_t fat_file_addr_resolve(struct file_s* file);
static fstatus fat_make_entry_chain(struct dir_s* dir, uint8_t entry_cnt);
...
static void fat_kprint_table(struct volume_s* vol, uint32_t sector) {
	...
	for (uint8_t i = 0; i < 128;) {
		...
		// kprinti((uint8_t)(curr >> 24));
		// kprint("\n");
		// kprinti((uint8_t)(curr >> 16));
		// kprint("\n");
		// kprinti((uint8_t)(curr >> 8));
		// kprint("\n");
		// kprinti((uint8_t)(curr));
		// kprint("   ");
		if ((i++ % 4) == 0) {
			kprint("\n");
			kprint("FAT: ");
...
static uint8_t fat_memcmp(const void* src_1, const void* src_2, uint32_t count) {
	...
	const uint8_t* src_1_ptr = (const uint8_t *)src_1;
	const uint8_t* src_2_ptr = (const uint8_t *)src_2;
...
static uint8_t fat_search(const uint8_t* bpb) {
	...
	uint32_t root_sectors = ((fat_load16(bpb + BPB_ROOT_ENT_CNT) * 32) + 
		(fat_load16(bpb + BPB_SECTOR_SIZE) - 1)) / 
		(fat_load16(bpb + BPB_SECTOR_SIZE) - 1);
	
	uint32_t fat_size = (fat_load16(bpb + BPB_FAT_SIZE_16)) ? 
		(uint32_t)fat_load16(bpb + BPB_FAT_SIZE_16) : 
		fat_load32(bpb + BPB_32_FAT_SIZE);
		
	uint32_t tot_sect = (fat_load16(bpb + BPB_TOT_SECT_16)) ? 
		(uint32_t)fat_load16(bpb + BPB_TOT_SECT_16) :
		fat_load32(bpb + BPB_TOT_SECT_32);
		
	uint32_t data_sectors = tot_sect - (fat_load16(bpb + BPB_RSVD_CNT) + 
...
static uint8_t fat_table_get(struct volume_s* vol, uint32_t cluster, uint32_t* fat_entry) {
...
static uint8_t fat_get_cluster(struct volume_s* vol, uint32_t* cluster) {
	// Load the FSinfo sector
	if (!fat_read(vol, vol->fsinfo_lba)) {
		return 0;
	}
	uint32_t next_free = fat_load32(vol->buffer + INFO_NEXT_FREE);
	uint32_t tot_free = fat_load32(vol->buffer + INFO_CLUST_CNT);
...
uint8_t disk_mount(disk_e disk) {
	...
	for (uint8_t i = 0; i < 4; i++) {
		uint32_t offset = MBR_PARTITION + i * MBR_PARTITION_SIZE;
		...
		if (partitions[i].lba) {
			...
			if (fat_search(mount_buffer)) {
				...
				struct volume_s* vol = (struct volume_s *)
					find_memblock(0x00000000, sizeof(struct volume_s));
