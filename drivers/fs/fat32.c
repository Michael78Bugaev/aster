    ...
    if(!*nextentry) {
        ...
        *secondcluster = get_next_cluster_id(fs, cluster);
        if(*secondcluster >= EOC) {
            ...
            kprintc("panic: FOUND BAD DIRECTORY ENTRY!", 0x0C);
        }
        ...
        *nextentry = read_dir_entry(fs, entry, end_of_cluster + fs->cluster_size, target_dirent);
        if(!*nextentry) {
            ...
            kprintc("panic: FAILED TO READ DIRECTORY ENTRY! THE SOFTWARE IS BUGGY!\n", 0x0C);
        }
    }
}
...
//               dir->entries[i].file_size, dir->entries[i].first_cluster);
        ...
        kprint(namebuff);
        kprint(" ");
        kprint(dir->entries[i].dir_attrs & DIRECTORY?'D':' ');
        kprint(" ");
        kprinti(dir->entries[i].file_size);
        ...
        while(1) {
            ...
            if(cluster >= EOC) break;
            if(cluster == 0) {
                kprintc("panic: BAD CLUSTER CHAIN! FS IS CORRUPT!", 0x0C);
            }
            cluster_count++;
        }
        kprint("clusters: [");
        kprinti(cluster_count);
        kprint("]\n");
    }
    mfree(namebuff);
}
