...
static vfs_info_t devfs_vfs_info = { .tag = "devfs",
                              .name = "AsterOS Device Meta System",
...
void devfs_init () {
  ...
  strncpy (&devfs_nodes, &devfs_default_layout, sizeof (devfs_default_layout));
 ...
 */
int devfs_readdir (vfs_node_t *dirnode, uint32_t index, vfs_dirent_t *target_dirent) {
  int i, found;

//  printf ("devfs_readdir(%s, %d)\n", dirnode->name, index);
...
//    printf ("Comparing inode slot %d: %d => %d\n", i, devfs_nodes[i].parent_inode_nr, dirnode->inode_nr);
    ...
    if (devfs_nodes[i].parent_inode_nr == dirnode->inode_nr) {
      ...
      index--;
//      printf ("Found file (%s) in dir dec index to %d\n", devfs_nodes[i].name, index);
      if (index == 0) {
        found = i;
        break;
      }
...
//  printf ("devfs_finddir (%s, %s);\n", dirnode->name, name);

  // Get the directory ID where this file resides in
  index = 0;
  uint32_t parent_inode = devfs_nodes[dirnode->inode_nr].inode_nr;
  for (i=1; i!=DEVFS_MAX_FILES; i++) {
    // This entry is empty
    if (devfs_nodes[i].type == 0) continue;
