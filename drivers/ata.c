...
static uint8_t ide_read_register(uint8_t channel, uint8_t reg) {
    ...
    if (reg < 0x08)
        ret = port_byte_in(g_ide_channels[channel].base + reg - 0x00);
    else if (reg < 0x0C)
        ret = port_byte_in(g_ide_channels[channel].base + reg - 0x06);
    else if (reg < 0x0E)
        ret = port_byte_in(g_ide_channels[channel].control + reg - 0x0A);
    else if (reg < 0x16)
        ret = port_byte_in(g_ide_channels[channel].bm_ide + reg - 0x0E);
...
static void ide_write_register(uint8_t channel, uint8_t reg, uint8_t data) {
    ...
    if (reg < 0x08)
        port_byte_out(g_ide_channels[channel].base + reg - 0x00, data);
    else if (reg < 0x0C)
        port_byte_out(g_ide_channels[channel].base + reg - 0x06, data);
    else if (reg < 0x0E)
        port_byte_out(g_ide_channels[channel].control + reg - 0x0A, data);
    else if (reg < 0x16)
        port_byte_out(g_ide_channels[channel].bm_ide + reg - 0x0E, data);
